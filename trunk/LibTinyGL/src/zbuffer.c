...
void ZB_copyFrameBufferRGB32_OneLine(unsigned char *pSrc, unsigned char *buf, int nNumPixel)
{
	unsigned short *q;
	unsigned int *p, *p1, v, w0, w1;
	int n;
	...
	q = (unsigned short *) pSrc;
	p1 = (unsigned int *) buf;

	p = p1;
	n = nNumPixel >> 2;
	nRemain = nNumPixel - (n<<2);
	...
	{
		v = *(unsigned int *) q;
		p[0] = ((v & 0xF800) << 8) | ((v & 0x07E0) << 5) | ((v & 0x001F) << 3);
	}
}
...
void ZB_copyFrameBufferRGB24_OneLine(unsigned char *pSrc, unsigned char *buf, int nNumPixel)
{
	unsigned short *q;
	unsigned int *p, *p1, w0, w1, w2, v0, v1;
	unsigned char *pLast;
	...
	pLast = (unsigned char*)p;
	for (; nRemain>0; nRemain--)
	{
		pLast[0] = (unsigned char)(0xFF & ((q[0] & 0xF800) >> 8));
		pLast[1] = (unsigned char)(0xFF & ((q[0] & 0x07E0) >> 3));
		pLast[2] = (unsigned char)(0xFF & ((q[0] & 0x001F) << 3));
...
// Notes: linesize is in BYTES
void ZB_copyFrameBufferWithOffset(ZBuffer * zb, void *buf, int linesize, unsigned short xOffset, unsigned short yOffset)
{
    switch (zb->mode)
    {
#ifdef TGL_FEATURE_8_BITS
    case ZB_MODE_INDEX:
		{
			ZB_ditherFrameBuffer(zb, ((unsigned char*)buf) + (linesize >> 1) * yOffset + xOffset, linesize >> 1);
		}
		break;
#endif
    ...
    case ZB_MODE_5R6G5B:
		{
			ZB_copyBuffer(zb, ((PIXEL*)buf) + (linesize >> 1) * yOffset + xOffset, linesize);
		}
		break;
#endif
#ifdef TGL_FEATURE_32_BITS
    case ZB_MODE_RGBA:
		{
			ZB_copyFrameBufferRGB32(zb, ((int*)buf) + (linesize >> 1) * yOffset + xOffset, linesize >> 1);
		}
		break;
#endif
#ifdef TGL_FEATURE_24_BITS
    case ZB_MODE_RGB24:
		ZB_copyFrameBufferRGB24(zb, ((int*)buf) + (linesize >> 1) * yOffset + xOffset, linesize >> 1);
		break;
#endif
    default:
    	assert(0);
